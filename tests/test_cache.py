...
def clear_caches():
    from utils.cache import _project_cache, _project_membership_cache, _user_projects_cache
...
def test_get_project_returns_cached_on_second_call(app, mock_client):
    from utils.cache import get_project
    fake_data = {'samples': [], 'datasets': []}
    with patch('utils.project_graph.generate_project_cache', return_value=fake_data) as mock_gen:
        with app.test_request_context('/'):
            get_project('proj-a', 'orcid-1', client=mock_client)
            get_project('proj-a', 'orcid-1', client=mock_client)
...
def test_clear_project_cache_all_orcids(app, mock_client):
    from utils.cache import get_project, clear_project_cache, _project_cache
    fake_data = {'samples': [], 'datasets': []}
    with patch('utils.project_graph.generate_project_cache', return_value=fake_data):
        with app.test_request_context('/'):
    ...
    clear_project_cache('proj-a')
    assert ('orcid-1', 'proj-a', False) not in _project_cache
    assert ('orcid-2', 'proj-a', False) not in _project_cache
